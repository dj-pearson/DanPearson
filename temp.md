...
async function fetchAmazonProducts(niche, itemCount = 5, partnerTag, supabaseClient, minIntervalMs = 1100) {
  ...
  for(let attempt = 0; attempt < maxAttempts; attempt++){
    ...
    try {
      const { data: claim } = await supabaseClient.rpc('claim_amazon_throttle', {
...
async function fetchSEOData(keyword) {
...
async function generateArticleContent(products, niche, seoData, wordCount) {
  ...
  const response = await fetch('https://ai.gateway.lovable.dev/v1/chat/completions', {
    ...
    body: JSON.stringify({
      ...
      messages: [
        {
          role: 'system',
          content: 'You are an expert product reviewer and SEO content writer. Always return valid JSON only.'
        },
        {
          role: 'user',
          content: prompt
...
serve(async (req)=>{
  ...
  try {
    ...
    const { data: running } = await supabase.from('amazon_pipeline_runs').select('id, started_at').eq('status', 'running').gte('started_at', tenSecondsAgo).limit(1);
    ...
    // Create run record
    const { data: run, error: runError } = await supabase.from('amazon_pipeline_runs').insert({
      status: 'running'
    }).select().single();
    if (runError) throw runError;
    const runId = run.id;
    const log = async (level, message, ctx)=>{
    ...
    // Fetch settings
    const { data: settingsRow } = await supabase.from('amazon_pipeline_settings').select('*').maybeSingle();
    ...
    await log('info', 'Settings loaded', {
      ...
      override_cache_only: Boolean(bodyOverrides?.cacheOnly),
      effectiveCacheOnly
    });
    // Pick random niche
    ...
    const { data: cached, error: cacheError } = await supabase.from('amazon_products').select('asin, title, brand, rating, rating_count, price, image_url, bullet_points').eq('niche', niche).gte('last_seen_at', twentyFourHoursAgo).order('rating', {
      ascending: false
    }).limit(5);
    let products = [];
    if (cacheError) {
      await log('warn', 'Cache query failed', {
        error: cacheError.message
      });
    }
    if (cached && cached.length) {
      products = cached.map((c)=>({
          asin: c.asin,
          title: c.title,
          brand: c.brand,
          rating: c.rating || 0,
      ...
      products = products.filter((p)=>p.rating >= (settings.min_rating || 0) && p.price > 0 && (settings.price_min ? p.price >= settings.price_min : true) && (settings.price_max ? p.price <= settings.price_max : true));
    ...
    if (products.length < 3) {
      ...
      if (!effectiveCacheOnly) {
        ...
        try {
          ...
          products = amazonData.SearchResult.Items.map((item)=>({
              asin: item.ASIN,
            ...
            })).filter((p)=>p.rating >= (settings.min_rating || 0) && p.price > 0 && (settings.price_min ? p.price >= settings.price_min : true) && (settings.price_max ? p.price <= settings.price_max : true));
          if (products.length === 0) {
            // Fallback to older cache (7 days)
            const sevenDaysAgo = new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString();
            const { data: oldCache } = await supabase.from('amazon_products').select('asin, title, brand, rating, rating_count, price, image_url, bullet_points').eq('niche', niche).gte('last_seen_at', sevenDaysAgo).order('rating', {
              ascending: false
            }).limit(5);
            if (oldCache && oldCache.length) {
              products = oldCache.map((c)=>({
                  asin: c.asin,
                  title: c.title,
                  brand: c.brand,
                  rating: c.rating || 0,
          ...
          await log('warn', 'Amazon API failed; using fallback cache', {
    ...
    if (products.length === 0) {
      const message = effectiveCacheOnly ? 'No cached products for this niche. Disable cache-only mode or seed products first.' : 'Amazon throttled; no products available and cache empty';
      await supabase.from('amazon_pipeline_runs').update({
        status: 'partial',
    ...
    const articleData = await generateArticleContent(products, niche, seoData, settings.word_count_target);
    // Create slug
    ...
    await supabase.from('amazon_pipeline_settings').update({
